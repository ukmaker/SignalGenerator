 ...
 * - 9.43 V
 */
void ValueWidget::_draw(Display& display) {
...
void ValueWidget::handleJoystickYMove(int events) {
  int deltaV = 0;
...
void ValueWidget::handleJoystickXMove(int events) {
  int deltaI = 0;
  ...
  updateIncrement(deltaI);
}

void ValueWidget::handleJoystickClick() {
   setToDecade();    
}

void ValueWidget::handleEncoderScroll(int steps) {
   updateValue(steps);   
}